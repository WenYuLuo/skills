---
name: yr-dev
description: Use for any openYuanRong repository work, especially before committing, pushing, creating or updating a GitCode MR, or when project-specific build, package, dependency, cache, deployment, and validation guidance is needed
---
...
处理 YuanRong/openYuanRong 相关任务时，先按任务类型路由，不要从通用 Linux/Docker/Bazel/CMake 经验直接摸索：

| 任务 | 优先入口 | 说明 |
|---|---|---|
| 本地编译、出包、依赖下载、缓存、容器网络 | 本 skill 的“网络稳定编译” + `references/build-network-robustness.md` | 先稳定网络/cache/env，再判断构建语义问题。 |
| 本地 ARM/GCC11 或 Python package 已知 build-support 问题 | `references/yuanrong-local-build-patches.md` | 只作为 build-support 层，不和产品功能混 diff。 |
| AIO/单容器运行验证 | `yr-smoke-aio` 或 `yuanrong-aio` skill | 用于运行、部署、smoke/e2e，不作为默认编译环境。 |
| 本地 3VM/Lima process-mode smoke/e2e | `yr-local-3vm` skill | 本地三节点 VM 是通用部署、运行、e2e、故障与性能验证环境；编译仍由本 skill/builder 负责。不要误路由到远端 dedicated 3VM。 |
| process-mode / 本地进程 smoke | `yr-process-smoke` skill | 用于进程模式最小验证。 |
| dedicated 3VM 远端 smoke | `yr-dedicated-cluster-smoke` skill | 只在本地闭环后或本地无法验证时做裁决。 |
| 新建/恢复云上 3VM/ECS 集群 | `building-yr-smoke-ecs` skill | 只用于搭建新的专用 smoke 集群。 |
| Buildkite/CI 调度与产物裁决 | `yr-buildkite` skill | 不替代本地编译闭环；用于 CI/远端验证和 Buildkite 专项流程。 |
| GitCode PR/CLA/MR 操作 | 本 skill 的“GitCode 平台”章节 | commit 必须满足 Signed-off-by/CLA 规则。 |

默认分工：编译/出包优先使用标准 Linux 编译容器或 local-build wrapper；AIO/3VM/VM 默认用于运行、e2e、smoke、压测；Buildkite/远端默认后置。

本地 3VM 术语约定：默认指本机/LAN 的三节点 VM（例如 Lima 三节点），不是华为云 dedicated 3VM。远端 dedicated 3VM 只用于本地闭环后的最终回归/裁决，或本地环境明确不可验证时的后置裁决。

本地 3VM/Lima 常见坑：
- 本地 VM 可能因文件系统只读、SSH 不可达、VZ 启动卡住等原因不可用；先采集 `limactl list`、磁盘、挂载和 SSH 证据，再决定修复或替换节点。
- `limactl clone` 通常要求源实例停止；不要 clone running instance。
- Python wheel 验证要确认安装位置，避免 user-site 污染系统 site-packages；必要时清理 user-site 后用目标 Python 的系统 site-packages 重装。
- macOS 主机脚本不要假设有 GNU `timeout`；Linux VM 内通常可用。
- 主机 shell 可能是 zsh；管道退出码不要误用 Bash `PIPESTATUS`，要用 Bash 包裹或使用 zsh `$pipestatus`。
- 本地 3VM/process-mode 反复部署会累积 `/tmp/yr_sessions`、`/testpythonpayh`、`/testEnv` 等运行暂存；若出现 `No space left on device`、etcd quorum 从 3 变 2、某节点 START NOT-OK，先检查 `df -h /` 和这些暂存目录，清理后再重跑，不要误判为产品代码或构建产物失败。
...
- `references/yuanrong-local-build-patches.md` — 当网络稳定前置后，full `make all` 仍在本地 ARM/GCC11 或 Python package 阶段失败时，读取这里的本地 build-support patch 层；该参考只绑定 repo/commit/失效判断，不绑定任何项目目录。
...
若网络/cache 前置后仍稳定复现 `functionsystem` C++17/GCC11、`setuptools/pkg_resources` 或 Python wheel 复制时序问题，读取 `references/yuanrong-local-build-patches.md`。这些处理属于本地 build-support patch 层，后续应独立 upstream 化，不得和产品功能改动混合。

## GitCode 平台

### 提交与 MR 强制门禁

任何 openYuanRong commit、push、创建 MR 或更新 MR 前都必须执行本节，不得用普通
`git commit` 或裸 GitCode API 绕过：

1. commit subject 使用 `type(scope): 描述`；Lore/决策记录放正文或 trailer，不能替代规范 subject；
2. 每个 commit 必须且只能有一行 CLA 邮箱对应的 `Signed-off-by`，禁止 `Co-authored-by`；
3. MR title 使用相同的 conventional 格式，并从目标仓库 `.gitee/PULL_REQUEST_TEMPLATE*`
   填写 `/kind`、目的、`Fixes #`、接口变化、测试/验证与 `[x]` 自检；
4. 创建或更新 MR 前运行 `scripts/preflight-pr.sh <repo> ...`，检查 `base..HEAD` 全部 commit，
   不能只检查最后一条；
5. MR 描述更新后用 `scripts/gitcode.sh check-pr <repo> <iid>` 校验并自动评论 `/check-pr`；
6. 优先使用 `submit.sh`、`create-pr.sh` 和 `gitcode.sh`，这些入口必须共用同一门禁。
...
curl -s -X POST "$API" \
  ...
  -d '{"title":"feat(frontend): ...","head":"branch-name","base":"master","body":"按仓库模板填写"}'
```

不要直接执行上述裸 API 示例作为日常路径；它仅说明 API 形态。日常使用
`scripts/create-pr.sh` 或 `scripts/gitcode.sh create-pr`，创建后自动评论 `/check-pr`。
webhook 会检查 Signed-off-by，不通过则 push 或 upstream PR 创建会被拒绝。
...
MR body 必须使用目标仓库模板，包含 `/kind`、`Fixes #`、接口变化、测试/验证和已勾选
自检项；脚本会在 commit 和 push 前拒绝不完整输入，并在创建成功后评论 `/check-pr`。

### preflight-pr.sh — 提交与 MR 审计

```bash
preflight-pr.sh frontend --title 'feat(frontend): support owner routing' \
  --body-file /tmp/frontend-mr.md --base upstream/master
preflight-pr.sh frontend --iid 98 --base upstream/master --comment-check-pr
```

检查 `base..HEAD` 全部 commit 的 subject、唯一 CLA signoff 和禁用 co-author，同时检查
MR title、`/kind`、Issue、接口、测试与 checklist。已有 MR 更新后必须运行第二种形式。
