---
name: yr-smoke-aio
description: Use to stand up a self-contained local multi-node openYuanRong cluster (master plus workers, each a privileged AIO container) and run isolated Python actor smoke cases against rebuilt Rust functionsystem binaries. Covers baking binaries into a current split-wheel or legacy AIO base, deployment, credential-safe case checkout, node health, and macOS Bash 3.2 compatibility. Complements yuanrong-aio single-container validation and yr-process-smoke remote validation.
---
...
cases against it. Use it for local multi-node behavior and fast Rust functionsystem iteration.

`scripts/yr-smoke-aio.sh` is the orchestrator (run with no args for usage).

## Dependencies

- **Repository credentials:** configure SSH or a Git credential helper before `fetch-cases`.
  Set `YR_SMOKE_CASES_REPO` to a URL without embedded credentials; never put a token in the URL.
- **Two Docker images:**
  - AIO **base** image (default `${YR_SMOKE_AIO_BASE:-yuanrong-aio:rust}`) — a complete
    single-container stack. A current local AIO image is also supported.
  - A **compile** image (default `compile-ubuntu2004-rust:arm64`) — cargo + protoc, arch/glibc-matched.
  - If you do not have them, `build-image` prints base-image guidance. Build the base
...
must share architecture and a compatible OS/glibc ABI. The image template discovers the active
Python site-packages and prefers `yr/functionsystem`, with legacy `yr/inner/functionsystem` fallback.
**Language replacement** — when fixing any bug/feature in the
rust binaries, mirror the C++ baseline (openYuanRong `feature/sandbox`), don't invent.

## Workflow
```bash
SKILL_DIR=/absolute/path/to/yr-smoke-aio
S="$SKILL_DIR/scripts/yr-smoke-aio.sh"
...
$S build-image /path/to/target/release  yr-local-aio:latest  yr-smoke-aio:local
# 2. deploy master + 2 workers (3-node)
$S up          yr-smoke-aio:local  3
# 3. pull cases using existing SSH/credential-helper authentication
export YR_SMOKE_CASES_REPO=git@gitcode.com:your-org/OpenYuanRongTest.git
$S fetch-cases ~/yr-smoke-cases
...
1. **macOS Bash 3.2:** run the bundled script directly; it intentionally avoids `mapfile` and other
   newer Bash-only features. Keep any future host-side array loading in `while IFS= read -r` form.
2. **Split-wheel paths:** never hard-code a CPython version or `yr/inner`. The image template resolves
   `yr/functionsystem` dynamically and falls back to legacy full-wheel layout only when present.
3. **docker cp drops the +x bit.** Worker `start-yuanrong.sh` MUST be executable; `up` chmods the HOST
...
4. **Disable the faas function_scheduler** (`--enable_function_scheduler false`, baked into start-master.sh).
   ...
   doesn't use it.
5. **Env inheritance into runc runtimes.** Deploy-time env (MY_ENV/LD_LIBRARY_PATH/PYTHONPATH) only
...
6. **Per-node custom_resources/labels** must match what the cases assert (node_tag1/2/3, name/role/
   number/only) — see start-master.sh / the worker template; mirror the remote `do_deploy.sh`.
7. **Test isolation.** Run each test FILE in its own pytest process (`run` does). Actor runtimes can
...
8. **Status endpoint:** derive the master container IP at runtime. Docker bridge addresses are not
   stable; never bake `172.17.x.x` into the script.

For Docker Desktop memory, ARM64 image architecture, nested dockerd `overlay2`→`vfs`, split
runtime wheels, and Go plugin ABI gates, read
`../yr-dev/references/macos-apple-silicon-aio-build.md` before rebuilding the base image.
...
- AIO uses **containerized (runc) runtimes** vs the remote **process-mode** — a known model difference;
  most cases align once env/labels are wired (gotcha #5/#6), the rest is scheduler debt above.
